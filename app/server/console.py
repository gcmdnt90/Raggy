...
def require_console(
    ...
    if not check.ok:
        return {"authenticated": False, "unprotected": True}
    ...
    if not secrets.compare_digest(supplied, expected):
        raise HTTPException(
            ...
            detail="Stage console password required.",
            headers={"WWW-Authenticate": 'Basic realm="Banco stage console"'},
        )
    return {"authenticated": True, "unprotected": False}
...
def get_preflight(
    ...
    report = preflight.report(get_settings(), sector, probe_ollama=probe)
...
def set_keys(update: KeyUpdate, auth: ConsoleAuth) -> dict:
    ...
    if update.ollama_base_url.strip():
        ...
        try:
            updates["ollama_base_url"] = normalize_ollama_base_url(update.ollama_base_url)
    ...
    if not updates:
        return {"saved": [], "detail": "Nothing to save."}
    ...
    return {"saved": sorted(updates), "detail": f"Saved {len(updates)} setting(s)."}
...
def clear_key(update: KeyUpdate, auth: ConsoleAuth) -> dict:
    ...
    if not cleared:
        return {"cleared": [], "detail": "Send the literal value CLEAR to blank a key."}
    save_settings_to_env(dict.fromkeys(cleared, ""))
    return {"cleared": cleared, "detail": f"Cleared {len(cleared)} key(s)."}
...
def run_check(auth: ConsoleAuth, sector: str = "numismatics") -> dict:
    ...
    """
    settings = get_settings()
    report = preflight.report(settings, sector, probe_ollama=True)
    report["live"] = preflight.live_checks(settings)
...
def pull_model(request: PullRequest, auth: ConsoleAuth) -> StreamingResponse:
    ...
    def stream():
        try:
            ...
            yield f'event: error\ndata: {{"error": "{type(exc).__name__}: {exc}"}}\n\n'
...
# TODO(M4): the rest of pre-flight — provider reachability with a real call,
#   sector chosen and persisted, recordings dated. See PROJECT.md milestone M4.
